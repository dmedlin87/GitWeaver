...
function parseInteger(value: string): number {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error(`Invalid integer value: ${value}. Expected a positive integer.`);
  }
  return parsed;
}
...
function parseBootstrapTemplate(value: string): BootstrapTemplate {
  ...
  throw new Error(`Invalid bootstrap template: ${value}. Allowed values: blank, web-game-ts`);
}
...
function parsePlannerProvider(value: string): ProviderId {
  ...
  throw new Error(`Invalid planner provider: ${value}. Allowed values: codex, claude, gemini`);
}
...
function parseForceModel(value: string): ProviderId {
  ...
  throw new Error(`Invalid force-model provider: ${value}. Allowed values: codex, claude, gemini`);
}
