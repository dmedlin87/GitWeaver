...
- Verified `ContextPack` selection is deterministic: ordered by tier priority (must, then should, then optional) and then by path (`localeCompare`) for tie-breaking. Byte budget enforcement occurs after deterministic ordering.
